    ...
    if (taskIndex !== null) {
      ...
      try {
        const taskData = {
          ...
          duedate: duedatedata,
          duetime: duetimedata,
          additional_notes: additional_notesdata
        };
...
// Alltask.html: Display all tasks from MongoDB
async function renderAllTasks() {
  ...
  try {
    ...
    if (priorityFilter && priorityFilter.value !== "all") {
      filtered = filtered.filter(t => t.level === priorityFilter.value);
    }
    ...
    if (sortBy) {
      ...
      if (sortBy.value === "priority") filtered.sort((a, b) => a.level.localeCompare(b.level));
        ...
        div.innerHTML = `
          <div class="task-header">
            ...
            <div class="task-status">
              <span class="status-badge ${task.status || 'pending'}">${(task.status || 'pending').charAt(0).toUpperCase() + (task.status || 'pending').slice(1)}</span>
              <span class="priority-badge ${task.level.toLowerCase()}">${task.level}</span>
            </div>
          </div>
          ...
          <div class="task-actions">
            ${(task.status || 'pending') === "pending" ? 
              ...
              `<button class="btn secondary" onclick="undoTask('${task._id}')">
                <i class="ri-arrow-go-back-line"></i> Undo
              </button>`
            }
            <button class="btn secondary" onclick="viewTaskDetails('${task._id}')">
              <i class="ri-eye-line"></i> View
            </button>
            <button class="btn warning" onclick="deleteTask('${task._id}')">
    ...
    if (pendingDisplayed) pendingDisplayed.textContent = filtered.filter(t => (t.status || 'pending') === "pending").length;
...
// Complete task function using API
async function completeTask(taskId) {
  try {
    ...
    if (response.ok) {
      ...
      const errorData = await response.json();
      alert(`Error: ${errorData.message || 'Failed to complete task'}`);
    }
  } catch (error) {
...
// Undo task completion using API
async function undoTask(taskId) {
  try {
    ...
    if (response.ok) {
      ...
      const errorData = await response.json();
      alert(`Error: ${errorData.message || 'Failed to undo task'}`);
    }
  } catch (error) {
...
// Delete task function using API
    ...
    if (response.ok) {
      ...
      modalBody.innerHTML = `
        <div class="task-detail">
          ...
          <p><strong>Priority:</strong> ${task.level}</p>
          ...
          ${task.additional_notes ? `<p><strong>Notes:</strong> ${task.additional_notes}</p>` : ""}
        </div>
      `;
...
async function renderDashboard() {
  ...
  try {
    ...
    tasks.slice(0, 5).forEach(task => {
      const div = document.createElement("div");
      div.className = "task-item" + (task.status === "completed" ? " completed" : "");
      div.innerHTML = `<strong>${task.title}</strong> <span style='color:gray;font-size:0.9em;'>(${task.level})</span>`;
    ...
    if (normalCount) normalCount.textContent = tasks.filter(t => t.level === "Normal").length;
    if (intermediateCount) intermediateCount.textContent = tasks.filter(t => t.level === "Intermediate").length;
    if (importantCount) importantCount.textContent = tasks.filter(t => t.level === "Important").length;
    if (pendingCount) pendingCount.textContent = tasks.filter(t => (t.status || 'pending') === "pending").length;
    if (todayCount) {
...
function exportCompletedTasks() {
  ...
  const csvContent = "data:text/csv;charset=utf-8," 
    ...
    + completedTasks.map(task => 
        `"${task.title}","${task.description}","${task.level}","${task.category}","${task.duedate || ''}","${new Date(task.completedAt).toLocaleDateString()}","${task.additional_notes || ''}"`
      ).join("\n");
